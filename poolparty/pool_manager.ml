...
let rec in_memory_table global_storage storage_mutex =
  ...
  | Message_passing.CheckIn child_pid ->
    ...
    Storage.change global_storage storage_mutex child_pid Storage.Ready
  (*
   ...
   *)
  | Message_passing.NewHolder item -> Holder.new_holder current_pid item
  (*
  ...
  | Message_passing.LockHolder requester_pid ->
    Storage.change global_storage storage_mutex requester_pid Storage.Buzy;
    send requester_pid (Message_passing.CheckOut requester_pid)
