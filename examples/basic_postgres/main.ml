...
let rec run_single_query index conn =
  ...
  let _ =
    ...
    | Error x ->
      print_endline (Dbcaml.Execution_result.execution_error_to_string x)
  in
...
let () =
  ...
  let pool_id = Dbcaml.start_link ~connections:2 driver |> Result.get_ok in

  run_single_query 0 pool_id;
  ()
